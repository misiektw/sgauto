...
class SGAuto(WND_CLASS, FORM_CLASS):
        ...
        if len(self.proctimes) > 1:
            self.lbAvgProcTime.setText(
                "Average proc time:  {:.2f}s".format(
                    sum(self.proctimes) / len(self.proctimes)
                )
            )
    ...
    def saveSet(self, path):
        try:
            ...
            self.logst(
                "Program may still work fine, but current settings will not be saved."
            )
        else:
    ...
    def process_files(self, tstamp, sgfList, bakPath):
        if self.already_processing == True:
            ...
            with zipfile.ZipFile(
                bakPath + "/" + zipfname, "w", compression=zipfile.ZIP_DEFLATED
            ) as myzip:
                for file in sgfList.keys():
                    ...
                    try:
                        ...
                        print(
                            "File {} fst: {} ts: {} lastts:{}".format(
                                file, fstamp, tstamp, self.SET["LastTS"]
                            )
                        )
                        if fstamp > tstamp:
                            tstamp = fstamp
        ...
        for plik in self.SET["SvPaths"].keys():
            try:
                ...
                if self.cbAutoRes.isChecked():
                    self.logst(
                        'File "{}" vanished. Autorestoring last save'.format(
                            os.path.basename(plik)
                        )
                    )
                    ...
                    self.logst(
                        'File "{}" vanished. Skipping this turn. Will try {} more times.'.format(
                ...
                self.logst(
                    "System error on {} modify time! Skipping this turn.".format(
                        basename(plik)
                    )
                )
                return False
    ...
    def populate_lwSGPaths(self, paths, init=False):
        if init:
            while self.lwSGPaths.count():
                self.lwSGPaths.takeItem(0)  # clear monitor file list
        count, missing = 0, 0
        plist = list(paths)
        for path in plist:
            if path not in self.SET["SvPaths"].keys() or init:
                try:
                    ...
                    self.logst(
                        "File not found. Remove it from list or restore save: %s"
    ...
    def on_bAddFiles_clicked(self):
        try:
            paths = QtWidgets.QFileDialog.getOpenFileNames(
                self, "Select files to backup", self.SET["AddFilesLast"]
            )[0]
        except KeyError:
            paths = QtWidgets.QFileDialog.getOpenFileNames(
                self, "Select files to backup"
            )[0]
        if len(paths) > 0:
    ...
    def on_bBakPath_clicked(self):
        ...
        bp = QtWidgets.QFileDialog.getExistingDirectory(
            self, "Select backup folder for your files.", str(self.SET["BakPath"])
        )
        if len(bp) > 0:
            self.SET["BakPath"] = bp
            self.populate_tabFList(self.SET["BakPath"])
    ...
    def on_bStart_clicked(self):
        if self.bStart.isChecked():
            ...
            self.saveSet(self.inipath)
            self.logst(
                "Monitoring selected files every %d seconds."
                % int(self.SET["Interval"] / 1000)
            )
    ...
    def on_bLoadSet_clicked(self):
        ...
        path, _ = QtWidgets.QFileDialog.getOpenFileName(
            self, "Select settings file", self.SET["BakPath"], filter
        )
        if len(path) > 0:
            self.loadSet(path)
        ...
        for f in myzip.infolist():
            ...
            if topdir:
                try:
                    ...
                    self.logst(
                        "Cannot extract: {}.\n\t   Maybe other program is blocking it...".format(
                ...
                self.logst(
                    "File {} does not match any path in the savelist. Skipping.".format(
                        fname
                    )
                )
    ...
    def restoreSave(self, fname):
        try:
            with zipfile.ZipFile(fname) as myzip:
                self.extractWithTS(
                    myzip
                )  # myzip.extractall() does not preserve timestamps so...
    ...
    def on_pbDelete_clicked(self):
        ...
        for id in [v.row() for v in selected]:
            ...
            try:
                os.rename(
                    fname[:-4] + ".comm.txt", ".trash/" + fname[:-4] + ".comm.txt"
                )
            except FileNotFoundError:
                pass
            tab.removeRow(id)
